...
//					Log.d(TAG, "message bytes " + bytes);
					Log.d(TAG, "message string bytes " + String.valueOf(bytesRead));
...
/////cust
					...
					String s = read(bytesRead , buffer);
					if (s.length() > 0)
						rx_buffer += s;
...
////////////////////////////////
		...
		private void sendToReadHandler(String s) {
			...
			msg.obj = s;
			mHandler.sendMessage(msg);
			mHandler.obtainMessage(2,0,0,s).sendToTarget();
			Log.i(TAG, "[RECV s] " + s);
		}
		/**
		 ...
		 * @param buffer
		 */
		private String read(int bytesRead, byte[] buffer) {
